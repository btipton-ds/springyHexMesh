...
namespace HexahedralMesher {
	...
	DumpObj::DumpObj(const Grid& grid, const std::string& path)
		: _grid(grid)
		, _path(path)
	{}

	void DumpObj::write(ostream& out, int minNumberOfClamps, int clampMask) const {
		vector<size_t> cellIndices;
		gatherClampedCells(minNumberOfClamps, clampMask, cellIndices);
		writeCells(out, cellIndices);
	}

	void DumpObj::write(const string& rootName, int minNumberOfClamps, int clampMask) const {
		string filename(rootName + ".obj");
		ofstream out(_path + filename);
		write(out, minNumberOfClamps, clampMask);
	}

	void DumpObj::writeFaces(std::ostream& out, const std::vector<GridFace>& faces) const {
	...
	void DumpObj::writeFaces(std::ostream& out, int minNumberOfClamps, int clampMask) const {
		vector<GridFace> faces;
		gatherClampedFaces(minNumberOfClamps, clampMask, faces);
		writeFaces(out, faces);
	}

	void DumpObj::writeFaces(const std::string& rootName, int minNumberOfClamps, int clampMask) const {
		string filename(rootName + ".obj");
		ofstream out(_path + filename);
		writeFaces(out, minNumberOfClamps, clampMask);
	}

	void DumpObj::write(const std::string& rootName, const std::vector<Vector3d>& pts) const {
		string filename(rootName + ".obj");
		ofstream out(_path + filename);
	...
	void DumpObj::writeCells(const std::string& filename, const std::vector<size_t>& cellIndices) const {
	...
	void DumpObj::writeCells(ostream& out, const std::vector<size_t>& cellIndices) const {
		vector<size_t> vertIndices;
		map<size_t, size_t> vertIdxMap;
	...
	void DumpObj::gatherClampedCells(int minNumberOfClamps, int clampMask, vector<size_t>& cellIndices) const {
		cellIndices.clear();
	...
	void DumpObj::gatherClampedFaces(int minNumberOfClamps, int clampMask, vector<GridFace>& faces) const {
		faces.clear();
	...
	void DumpObj::gatherEdges(const std::vector<size_t> cellIndices, std::set<GridEdge>& edges) const {
	...
	void DumpObj::gatherEdges(const std::vector<GridFace> faces, std::set<GridEdge>& edges) const {
	...
	void DumpObj::gatherFaces(const std::vector<size_t> cellIndices, std::set<SearchableFace>& faceSet) const {
		for (size_t cellIdx : cellIndices) {
	...
	void DumpObj::createMaps(const vector<size_t>& cellIndices, vector<size_t>& vertIndices, map<size_t, size_t>& vertIdxMap) const {
		vertIndices.clear();
		vertIdxMap.clear();
	...
	void DumpObj::createMaps(const vector<GridFace>& faces, vector<size_t>& vertIndices, map<size_t, size_t>& vertIdxMap) const {
		vertIndices.clear();
		vertIdxMap.clear();
	...
	void DumpObj::dumpVertsObj(ostream& out, const vector<size_t>& vertIndices) const {
		out << "#Vertices\n";
	...
	void DumpObj::dumpLinesObj(ostream& out, const set<GridEdge>& edges, const map<size_t, size_t>& vertIdxMap) const {
		out << "#Edges\n";
	...
	void DumpObj::dumpFacesObj(ostream& out, const set<SearchableFace>& faceSet, const map<size_t, size_t>& vertIdxMap) const {
		out << "#Faces\n";
