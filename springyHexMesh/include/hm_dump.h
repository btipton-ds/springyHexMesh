...
namespace HexahedralMesher {
	...
	class Dump {
	public:
		Dump(const Grid& grid, const std::string& path = "");
		...
		void gatherClampedCells(int minNumberOfClamps, int clampMask, std::vector<size_t>& cellIndices) const;
		void gatherClampedFaces(int minNumberOfClamps, int clampMask, std::vector<GridFace>& faces) const;
